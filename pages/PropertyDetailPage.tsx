...
const PropertyDetailPage: React.FC = () => {
    ...
    return (
        ...
        >
            ...
            <div className="p-4 lg:p-0 lg:grid lg:grid-cols-5 lg:gap-8">
                 {/* Desktop Gallery & Floorplan */}
                <div className="hidden lg:block lg:col-span-3 space-y-8 pt-8">
                     <div>
                        <h2 className="text-xl font-semibold text-orange-600 dark:text-golden-yellow mb-2">Photo Gallery</h2>
                        {property.images && property.images.length > 0 ? (
                            <div className="relative w-full h-[450px] rounded-lg overflow-hidden bg-gray-200 dark:bg-zinc-800 flex items-center justify-center">
                 ...
                 {/* Details Section (Mobile & Desktop) */}
                <div className="space-y-6 lg:col-span-2 lg:pt-8">
                    <div className="hidden lg:block">
                        <h1 className="text-4xl font-bold text-zinc-900 dark:text-white break-words">{property.title}</h1>
                        <p className="text-2xl text-orange-600 dark:text-golden-yellow font-semibold mt-2">{property.price}</p>
                    </div>
                        ...
                        <div>
                            ...
                            {property.images && property.images.length > 0 ? (
                                <div className="relative w-full h-64 rounded-lg overflow-hidden bg-gray-200 dark:bg-zinc-800 flex items-center justify-center">
                                    ...
                                    <button onClick={() => paginate(-1)} className="absolute top-1/2 left-2 -translate-y-1/2 bg-black/50 p-2 rounded-full text-white hover:bg-black/80 transition-colors z-10"><ChevronLeftIcon className="h-6 w-6" /></button>
                                    <button onClick={() => paginate(1)} className="absolute top-1/2 right-2 -translate-y-1/2 bg-black/50 p-2 rounded-full text-white hover:bg-black/80 transition-colors z-10"><ChevronRightIcon className="h-6 w-6" /></button>

                                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                                        {property.images.map((_, i) => (
                                            <button key={i} onClick={() => handleDotClick(i)} className={`h-2.5 w-2.5 rounded-full transition-colors ${i === imageIndex ? 'bg-white' : 'bg-white/50 hover:bg-white/75'}`} />
                                        ))}
                                    </div>
                                </div>
                            ) : (
             ...
             <div className="fixed bottom-20 left-0 right-0 p-4 bg-white/80 dark:bg-charcoal/80 backdrop-blur-sm max-w-lg mx-auto lg:hidden">
                <CTAEnquire />
            </div>
        </motion.div>
    );
