...
const Sidebar: React.FC<SidebarProps> = ({ onClose }) => {
    ...
    const NavItem: React.FC<{ path: string, name: string, icon: React.ElementType }> = ({ path, name, icon: Icon }) => (
        <NavLink
            to={path}
            onClick={onClose}
            className={({ isActive }) => `flex items-center gap-4 px-4 py-3 rounded-lg transition-colors text-lg ${isActive ? 'bg-golden-yellow text-charcoal font-semibold' : 'hover:bg-gray-200 dark:hover:bg-zinc-700'}`}
        >
